...
class Packet
{
	public:
		...
		size_t Send(int i_currentTic, int i_destinationTic, SequenceSender& i_sender, const netadr_t& i_dest, NetDemo* netdemo = nullptr);
		size_t ReSend(int i_historicalLocalTic, int i_destinationTic, int sequence, const buf_t& i_dataBuffer, const netadr_t& i_dest);
		size_t SendHighPriority(int i_currentTic, int i_destinationTic, SequenceSender& i_sender, const netadr_t& i_dest, NetDemo* netdemo = nullptr);
