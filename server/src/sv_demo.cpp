...
void ServerNetDemo::copy(ServerNetDemo &to, const ServerNetDemo &from)
{
...
ServerNetDemo::ServerNetDemo(const ServerNetDemo &rhs)
{
	copy(*this, rhs);
}

ServerNetDemo& ServerNetDemo::operator=(const ServerNetDemo &rhs)
{
	copy(*this, rhs);
	return *this;
}
 ...
 * @param message Error message.
 */
void ServerNetDemo::fatalError(const std::string &message)
{
	cleanUp();
	PrintFmt(PRINT_HIGH, "{}\n", message);
}
...
bool ServerNetDemo::writeHeader()
{
...
{
	if (last_map_tic == 0 || gamestate != GS_LEVEL)
		return false;
...
void ServerNetDemo::writeSnapshotInterval()
{
...
void ServerNetDemo::writeMessages(int num_players)
{   
	auto output_buf = std::make_unique<byte[]>(captured.size() * (MAX_UDP_PACKET *  num_players));
...
{
    // should be able to write this easily from odasrv without "faking it" which is whats done here
...
void ServerNetDemo::writeConnectionSequence(buf_t *netbuffer)
{
	...
	MSG_WriteSVCBuffer(netbuffer, SVC_ConsolePlayer(nullplayer, "2"));

	// our userinfo
	MSG_WriteSVCBuffer(netbuffer, SVC_UserInfo(nullplayer, 4));
	...
	MSG_WriteSVCBuffer(netbuffer, SVC_PlayerMembers(nullplayer, SVC_PM_SPECTATOR));
	...
	MSG_WriteSVCBuffer(netbuffer, SVC_SpawnPlayer(nullplayer));
}
